...
namespace MeticulousCraft.Core
{
    ...
    public class NPCController : MonoBehaviour
    {
        ...
        public void SeekCompanionship(int time)
        {
            StartCoroutine(PathToFriendCoroutine(time));
        }
        ...
        IEnumerator PathToFriendCoroutine(int time)
        {
            int counter = time;
            while (counter > 0)
            {
            ...
            Debug.Log("I sought a friend and gained 1 energy!");
            // Logic to update energy
            stats.lonliness = 50;
